...
final class Installer
{
    public static function installSchema(): void
    {
        ...
        $courses = Database::table('courses');
        $courseModules = Database::table('course_modules');
        $courseLessons = Database::table('course_lessons');
        $lessonResources = Database::table('lesson_resources');
        $courseEnrollments = Database::table('course_enrollments');
        $lessonProgress = Database::table('lesson_progress');
        $certificates = Database::table('certificates');
        $queries = [
            ...
            "CREATE TABLE {$courseLessons} (
                ...
                KEY module_order (module_id,sort_order),
                KEY course_status (course_id,status)
            ) {$charsetCollate};",
            "CREATE TABLE {$lessonResources} (
                ...
                KEY lesson_order (lesson_id,sort_order),
                KEY status (status)
            ) {$charsetCollate};",
            ...
            "CREATE TABLE {$certificates} (
                ...
                workload_minutes_snapshot int(10) unsigned NOT NULL DEFAULT 0,
